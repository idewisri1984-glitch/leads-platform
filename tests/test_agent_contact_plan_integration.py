...
from app.modules.agent.contact_plan import AgentContactPlanProviderError
...
class FakeProvider:
    ...
    def __init__(self, company_id: int, fail: bool = False) -> None:
...
@pytest.fixture
def database(tmp_path) -> Generator[tuple[sessionmaker[Session], int, int]]:
...
def test_full_plan_persists_only_staging_and_is_idempotent(database) -> None:
    factory, project_id, company_id = database
    ...
    first = execute_agent_contact_plan(
        data, session_factory=factory, provider_factory=lambda: provider
    )
...
def test_provider_failure_rolls_back_external_database(database) -> None:
    factory, project_id, company_id = database
    with pytest.raises(AgentContactPlanProviderError):
        execute_agent_contact_plan(
            ...
            provider_factory=lambda: FakeProvider(company_id, fail=True),
        )
    with factory() as session:
